...
export async function getSessionToken(): Promise<string | null> {
  try {
    // Try to get VibeOn token first (from AsyncStorage)
    try {
      const AsyncStorage = require('@react-native-async-storage/async-storage').default;
      const vibeToken = await AsyncStorage.getItem('vibeon_token');
      if (vibeToken) {
        console.log("[Auth] VibeOn token found:", vibeToken.substring(0, 20) + "...");
        return vibeToken;
      }
    } catch (e) {
    ...
    // Use SecureStore for native (OAuth token)
